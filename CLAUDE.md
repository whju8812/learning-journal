...
4. **Query the database for recent sessions** to avoid repeating themes:
   - Use WebFetch to call `GET https://learning-journal.mariposa-lw.xyz/api/entries/recent?days=7`
   - If successful (HTTP 200), read the `tech_content` and `tech_application` fields from ALL returned sessions (covers last 7 days / up to 14 sessions) and extract already-covered topics, tools, and frameworks
   - If the request fails or returns an empty array, fall back to reading any local `.github/workflows/write-journal-entry-{YYYYMMDD}-*.yml` files and parsing their embedded JSON
   - Do NOT repeat any tool name, framework, or main theme already covered in the last 7 days
